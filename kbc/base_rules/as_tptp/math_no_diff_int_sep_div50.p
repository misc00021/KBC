...
cnf(rule_16, axiom, (is_not_zero(Y) & is_not_zero(X)) => '/'(X, '/'(X, Y)) = Y).
cnf(rule_17, axiom, (is_not_zero(Y)) => '/'('*'(X, Y), Y) = X).
cnf(rule_18, axiom, (is_not_zero(X) & is_not_zero(Y)) => '/'('/'(X, Y), X) = '/'('1', Y)).
cnf(rule_19, axiom, (is_not_zero(Z) & is_not_zero(Y)) => '/'('/'(X, Y), Z) = '/'('/'(X, Z), Y)).
cnf(rule_20, axiom, (is_not_zero(X)) => '/'('/'('1', X), X) = pow('/'('1', X), '2')).
cnf(rule_21, axiom, (is_not_zero(X)) => '/'(pow(X, '2'), X) = X).
cnf(rule_22, axiom, (is_not_zero(X)) => pow(X, '+'(Y, Y)) = pow(pow(X, Y), '2')).
cnf(rule_23, axiom, pow(X, '+'('1', '1')) = pow(X, '2')).
cnf(rule_24, axiom, pow(X, '+'(Y, Z)) = pow(X, '+'(Z, Y))).
cnf(rule_25, axiom, pow('1', '+'(X, '1')) = pow('1', X)).
cnf(rule_26, axiom, pow('1', '+'(X, '2')) = pow('1', X)).
cnf(rule_27, axiom, (is_not_zero(X)) => '*'(pow(X, Y), pow(X, Z)) = pow(X, '+'(Y, Z))).
