...
cnf(rule_10, axiom, (is_not_zero(Z) & is_not_zero(Y)) => '*'(X, '/'(Y, Z)) = '/'(X, '/'(Z, Y))).
...
cnf(rule_16, axiom, (is_not_zero(X) & is_not_zero(Y)) => '/'(X, '*'(Y, X)) = '/'('1', Y)).
cnf(rule_17, axiom, (is_not_zero(Y) & is_not_zero(X)) => '/'(X, '/'(Y, X)) = '/'(pow(X, '2'), Y)).
...
cnf(rule_21, axiom, (is_not_zero(Y) & is_not_zero(Z) & is_not_zero(X)) => '/'(X, '/'(Y, Z)) = '/'(Z, '/'(Y, X))).
cnf(rule_22, axiom, (is_not_zero(Z) & is_not_zero(Y)) => '/'('*'(X, Y), Z) = '/'(X, '/'(Z, Y))).
cnf(rule_23, axiom, (is_not_zero(Y)) => '/'('/'(X, Y), Y) = '/'(X, pow(Y, '2'))).
cnf(rule_24, axiom, (is_not_zero(Z) & is_not_zero(Y)) => '/'('/'(X, Y), Z) = '/'(X, '*'(Y, Z))).
...
cnf(rule_37, axiom, (is_not_zero(X)) => pow(X, '+'('-1', '-1')) = '/'('1', pow(X, '2'))).
cnf(rule_38, axiom, '*'(X, '*'(X, pow(Y, '2'))) = pow('*'(X, Y), '2')).
cnf(rule_39, axiom, (is_not_zero(X)) => '*'(X, '*'(Y, pow(X, Z))) = '*'(Y, pow(X, '+'(Z, '1')))).
cnf(rule_40, axiom, (is_not_zero(X)) => '*'(X, '*'(Y, pow(X, Z))) = '*'(pow(X, '+'(Z, '1')), Y)).
...
cnf(rule_44, axiom, (is_not_zero(Y)) => '*'(pow(Y, '+'(Z, Z)), X) = '*'(X, pow(pow(Y, Z), '2'))).
cnf(rule_45, axiom, (is_not_zero(X)) => '*'(pow(X, Y), pow(X, Z)) = pow(X, '+'(Y, Z))).
cnf(rule_46, axiom, '*'(pow(X, '2'), pow(Y, '2')) = pow('*'(X, Y), '2')).
cnf(rule_47, axiom, (is_not_zero(X) & is_not_zero(Y)) => '/'(X, '/'(Y, '/'(X, Y))) = pow('/'(X, Y), '2')).
cnf(rule_48, axiom, (is_not_zero(Z) & is_not_zero(W) & is_not_zero(X) & is_not_zero(Y)) => '/'(X, '/'(Y, '*'(Z, W))) = '/'(Z, '/'(Y, '*'(X, W)))).
cnf(rule_49, axiom, (is_not_zero(Y) & is_not_zero(X)) => '/'(X, '/'(pow(X, '2'), Y)) = '/'(Y, X)).
cnf(rule_50, axiom, (is_not_zero(Y) & is_not_zero(X)) => '/'(X, '/'(pow(Y, '2'), X)) = pow('/'(X, Y), '2')).
cnf(rule_51, axiom, (is_not_zero(Y) & is_not_zero(X)) => '/'(pow(X, '2'), '*'(Y, X)) = '/'(X, Y)).
cnf(rule_52, axiom, (is_not_zero(Y) & is_not_zero(Z) & is_not_zero(X)) => '/'(X, '/'(Y, '*'(X, Z))) = '/'(pow(X, '2'), '/'(Y, Z))).
cnf(rule_53, axiom, (is_not_zero(X)) => '/'(pow(X, '+'(Y, '1')), X) = pow(X, Y)).
cnf(rule_54, axiom, (is_not_zero(X)) => '/'(pow(X, '+'('1', Y)), X) = pow(X, Y)).
cnf(rule_55, axiom, (is_not_zero(X)) => '/'(pow('*'(X, Y), '2'), X) = '*'(X, pow(Y, '2'))).
cnf(rule_56, axiom, pow(X, '+'(Y, '+'('1', '1'))) = pow(X, '+'(Y, '2'))).
cnf(rule_57, axiom, (is_not_zero(X)) => pow(X, '+'('1', '+'(Y, Y))) = '*'(X, pow(pow(X, Y), '2'))).
...
cnf(rule_65, axiom, (is_not_zero(X)) => pow(X, '+'(Y, '+'(Y, Y))) = pow(pow(X, Y), '+'('2', '1'))).
cnf(rule_66, axiom, (is_not_zero(X)) => pow(pow(X, '+'(Y, Y)), '2') = pow(pow(X, Y), '+'('2', '2'))).
cnf(rule_67, axiom, pow(X, '+'(Y, '+'(Y, '2'))) = pow(pow(X, '+'(Y, '1')), '2')).
cnf(rule_68, axiom, (is_not_zero(X)) => pow(X, '+'('1', '+'(Y, '-1'))) = pow(X, Y)).
